...
{* Create view parameter text with the exception of the offset this pager owns.

   offset_name lets a page carry more than one pager: the node view already
   pages its sub items with (offset), so the locations tab asks for
   (location_offset) and the two no longer move each other. Left alone it is
   'offset', which is what every existing caller gets. *}
