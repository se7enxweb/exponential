...
{foreach $limit_choices as $limit_index => $limit_option}
    ...
    {else}
        <a href={concat( '/user/preferences/set/currencies_list_limit/', $limit_index|inc )|ezurl} title="{'Show %count items per page.'|i18n( 'design/admin/shop/currencylist',, hash( '%count', $limit_option ) )}">{$limit_option}</a>
    {/if}
{/foreach}
</p>
</div>
