...
class eZINITest extends ezpTestCase
{
    ...
    private static ?string $fixtureOriginal = null;

    public static function setUpBeforeClass(): void
    {
        $path = realpath( __DIR__ . '/../../../../' ) . '/settings/override/site.ini.append.php';
        self::$fixturePath = $path;

        if ( file_exists( $path ) )
        {
            self::$fixtureOriginal = file_get_contents( $path );
        }
        else
        {
    ...
    {
        if ( self::$fixturePath === null )
            return;

        if ( self::$fixtureOriginal !== null )
        {
            file_put_contents( self::$fixturePath, self::$fixtureOriginal );
        }
        else
        {
